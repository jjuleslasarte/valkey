...
# by polling INFO durability for reply_blocking_sync_replicas.
proc wait_for_isr_count {primary count} {
    wait_for_condition 100 100 {
        [getInfoProperty [$primary info reply_blocking] reply_blocking_sync_replicas] >= $count
    } else {
        fail "Expected $count sync replicas in ISR but got [getInfoProperty [$primary info reply_blocking] reply_blocking_sync_replicas]"
    }
}

# Helper: return the current write_blocked_count from INFO durability.
proc get_write_blocked_count {primary} {
    getInfoProperty [$primary info reply_blocking] reply_blocking_write_blocked_count
}
    ...
    start_server {overrides {sync-replication-enabled yes sync-eligible yes}} {
        ...
        start_server {overrides {sync-replication-enabled yes sync-eligible yes}} {
            ...
            test "Sync replication: write released only after both sync replicas ack" {
                ...
                $primary DEBUG reply-blocking-pause replication
    ...
    start_server {overrides {sync-replication-enabled yes sync-eligible yes}} {
        ...
        test "Sync replication: replica paused — writes rejected then resume after SIGCONT" {
            ...
            wait_for_condition 150 200 {
                [getInfoProperty [$primary info reply_blocking] reply_blocking_sync_replicas] == 0
            } else {
    ...
    start_server {overrides {sync-replication-enabled yes sync-eligible yes}} {
        ...
        start_server {overrides {sync-replication-enabled yes sync-eligible no}} {
            ...
            test "Sync replication: committed offset advances based on sync replica, not regular replica" {
                ...
                set offset_before [getInfoProperty [$primary info reply_blocking] reply_blocking_previous_acked_offset]
                ...
                set offset_after [getInfoProperty [$primary info reply_blocking] reply_blocking_previous_acked_offset]
                ...
                set offset_final [getInfoProperty [$primary info reply_blocking] reply_blocking_previous_acked_offset]
