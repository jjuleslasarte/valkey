 ...
 * in postCall(), and process them in postCommandExect() after the entire transaction is
...
// such as a MULTI/EXEC (or TODO: Lua)
...
static void freePendingUncommitedKey(void *uncommitted_key);
 ...
 * generated from executing the next Redis command comes after this position.
...
static void processPendingUncommittedData(long long blocking_repl_offset) {
    ...
    if (listLength(pending_uncommitted_keys) > 0) {
        ...
        while ((key_node = listNext(&li)) != NULL) {
            pendingUncommittedKey *uk = listNodeValue(key_node);
            addUncommittedKey(uk->key->ptr, blocking_repl_offset, uk->uncommitted_keys);
...
static void freePendingUncommitedKey(void *uncommitted_key) {
    decrRefCount(((pendingUncommittedKey*)uncommitted_key)->key);
    zfree(uncommitted_key);
}
